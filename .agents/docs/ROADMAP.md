...
- [x] [F027 — Plain-language pass (dual units, altitude, friendly warnings)](features/F027-plain-language-pass.md) — `S` `UI` (2026-09-10; `node --test` 24/24, lint + build clean, `grep "Domain box"` empty; one DECISIONS.md §F027 note on the comment-only sweep fix; 2 live-browser criteria outstanding — see spec notes)
- [x] [F028 — Live first paint (auto-load a sample at boot)](features/F028-auto-sample-boot.md) — `XS` `glue` (2026-09-10; code + lint/build clean with real wasm; 4 manual/browser criteria need a browser — see spec notes)
- [x] [F029 — Simple stats mode (hero numbers for non-experts)](features/F029-simple-stats-mode.md) — `S` `UI` (2026-09-10; code + `node --test`/lint/build clean + SSR-prerender verified; 3 live-browser criteria outstanding — see spec notes)
